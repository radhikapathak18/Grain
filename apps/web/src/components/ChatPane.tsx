...
import { QuestionShapeSelector } from './QuestionShapeSelector';
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-4 py-12">
      ...
      <h2 className="grain-display text-3xl sm:text-4xl font-semibold text-fg mb-3 max-w-2xl grain-fade-up grain-fade-up-delay-1">
        Ask your research a question.
      </h2>
      <p className="text-muted text-base max-w-lg mb-8 leading-relaxed grain-fade-up grain-fade-up-delay-2">
        Pick a shape, then ask. Every answer includes citations, trust signals, and per-product attribution.
      </p>
        ...
        {EXAMPLE_PROMPTS.map((p, i) => {
          ...
          return (
            <button
              ...
              className="grain-fade-up group text-left p-4 rounded-xl border border-border bg-bg hover:border-accent/40 hover:bg-surface/40 hover:-translate-y-0.5 grain-shadow-soft hover:grain-shadow-card transition-all duration-200 cursor-pointer focus:outline-none focus:ring-4 focus:ring-accent/15"
            >
    ...
    <div className="mt-4 flex flex-wrap gap-2">
      {suggestions.map((s) => (
        <button
          ...
          type="button"
          onClick={() => onPick(s)}
          className="text-xs px-2.5 py-1.5 rounded-full border border-border bg-bg hover:border-accent hover:text-accent transition-colors text-muted cursor-pointer"
        >
          {s}
        </button>
      ))}
...
export function ChatPane({ tabId, isActive }: Props) {
  const messages = useSessionStore((s) => {
    const tab = s.tabs.find((t) => t.id === tabId);
    return tab?.messages ?? [];
  });
  const setShape = useSessionStore((s) => s.setShape);
  const { send, streaming, error } = useChatStream(tabId);
  ...
  return (
    ...
    >
      ...
      {showFollowUps && (
        <FollowUpSuggestions asRole={lastMessage?.asRole} onPick={handleStarter} />
      )}
